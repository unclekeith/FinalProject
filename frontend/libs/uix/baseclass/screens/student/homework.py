...
class HomeworkScreen(MDScreen):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.documents = []
        self.current_open_document_id = None  # Track the ID of the currently opened document
        self.badge_states = {}  # Track the read/unread status of messages
        self.load_badge_states()  # Load previous badge states from file
    ...
    def get_all_documents(self, dt=None):
        """Fetch all documents from the server."""
        try:
            response = TeachersRoutes(client=self.manager.connection_client).list_files_teacher_list__get()

            # Ensure a successful HTTP response
            if response.status_code != 200:
                print(f"Error: Server returned status code {response.status_code}")
                self.show_error_message("Failed to retrieve documents. Please try again later.")
                return
            ...
            print(f"Error fetching documents: {e}")
            self.show_error_message("Error retrieving documents. Please check your connection.")
    ...
    def populate_all_documents(self):
        ...
        if not self.documents:
            self.ids.posts.add_widget(MDLabel(
                text="No Document received yet",
                bold=True,
                halign="center",
                padding=(dp(0), dp(200), 0, 0),
            ...
            for file in self.documents:
                ...
                def on_document_click(instance, file=file):
                    ...
                    self.save_badge_states()
                    self.populate_all_documents()  # Refresh the list
                    # Open Document Review Screen
                    self.manager.current = "document_review_screen"  # Switch to the document review screen

                list_item = MDListItem(
                    MDListItemLeadingIcon(icon="file-document"),
                    MDListItemHeadlineText(text=f"From: {self.first_name}"),
                    MDListItemSupportingText(id=f"supporting_{document_id}", text=document_name),
                    MDBadge(text=badge_text),  # Badge for unread documents
                    on_release=on_document_click,
                    id=f"item_{document_id}",
                    md_bg_color=(0, 0, 0, 0),  # Transparent background
    ...
    def download_file_teacher_download__filename__get(self, filename, **kwargs):
        """Download the file from the server."""
        try:
            response = TeachersRoutes(client=self.manager.connection_client).download_file_teacher_download__filename__get(filename, **kwargs)
            return response
        except Exception as e:
            print(f"Error downloading file: {e}")
            return None
...
class DocumentReviewScreen(MDScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.layout = BoxLayout(orientation='vertical')

        # Create a ScrollView for PDF page navigation
    ...
    def display_pdf(self, file_path):
        ...
        # Create an Image widget for each page
        for image in images:
            kivy_image = Image(texture=image.texture)
            self.scroll_view.add_widget(kivy_image)
